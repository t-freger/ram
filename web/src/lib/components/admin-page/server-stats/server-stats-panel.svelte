<script lang="ts">
  ...
  import type { ServerStatsResponseDto } from '@ram/sdk';
...
<div class="flex flex-col gap-5">
  <div>
    <p class="text-sm dark:text-ram-dark-fg">TOTAL USAGE</p>
    ...
    <div class="mt-5 flex lg:hidden">
      <div class="flex flex-col justify-between rounded-3xl bg-ram-gray p-5 dark:bg-ram-dark-gray">
        <div class="flex flex-wrap gap-x-12">
          ...
          <div class="relative text-center font-mono text-2xl font-semibold">
            <span class="text-[#DCDADA] dark:text-[#525252]">{zeros(stats.photos)}</span><span
              class="text-ram-primary dark:text-ram-dark-primary">{stats.photos}</span
            >
          </div>
        </div>
        <div class="flex flex-wrap gap-x-12">
          ...
          <div class="relative text-center font-mono text-2xl font-semibold">
            <span class="text-[#DCDADA] dark:text-[#525252]">{zeros(stats.videos)}</span><span
              class="text-ram-primary dark:text-ram-dark-primary">{stats.videos}</span
            >
          </div>
        </div>
        <div class="flex flex-wrap gap-x-7">
          ...
          <div class="relative flex text-center font-mono text-2xl font-semibold">
            <span class="text-[#DCDADA] dark:text-[#525252]">{zeros(statsUsage)}</span><span
              class="text-ram-primary dark:text-ram-dark-primary">{statsUsage}</span
            >
  ...
  <div>
    <p class="text-sm dark:text-ram-dark-fg">USER USAGE DETAIL</p>
    <table class="mt-5 w-full text-left">
      ...
      <tbody
        class="block max-h-[320px] w-full overflow-y-auto rounded-md border dark:border-ram-dark-gray dark:text-ram-dark-fg"
      >
        {#each stats.usageByUser as user (user.userId)}
          <tr
            class="flex h-[50px] w-full place-items-center text-center odd:bg-ram-gray even:bg-ram-bg odd:dark:bg-ram-dark-gray/75 even:dark:bg-ram-dark-gray/50"
          >
            ...
            <td class="w-1/4 text-ellipsis px-2 text-sm">
              ...
              {/if}
              <span class="text-ram-primary dark:text-ram-dark-primary">
                {#if user.quotaSizeInBytes}
