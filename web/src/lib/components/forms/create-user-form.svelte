<script lang="ts">
  ...
  import { createUser } from '@ram/sdk';
...
<FullScreenModal id="create-new-user-modal" title="Create new user" showLogo {onClose}>
  <form on:submit|preventDefault={registerUser} autocomplete="off" id="create-new-user-form">
    <div class="my-4 flex flex-col gap-2">
      ...
      <input class="ram-form-input" id="email" bind:value={email} type="email" required />
    </div>

    {#if $featureFlags.email}
      <div class="my-4 flex place-items-center justify-between gap-2">
        <label class="text-sm dark:text-ram-dark-fg" for="send-welcome-email"> Send welcome email </label>
    ...
    <div class="my-4 flex flex-col gap-2">
      <label class="ram-form-label" for="confirmPassword">Confirm Password</label>
      <PasswordField id="confirmPassword" bind:password={confirmPassword} autocomplete="new-password" />
    </div>

    <div class="my-4 flex place-items-center justify-between gap-2">
      <label class="text-sm dark:text-ram-dark-fg" for="require-password-change">
    ...
    <div class="my-4 flex flex-col gap-2">
      ...
      <input class="ram-form-input" id="name" bind:value={name} type="text" required />
    </div>
    ...
    <div class="my-4 flex flex-col gap-2">
      ...
      <input class="ram-form-input" id="quotaSize" type="number" min="0" bind:value={quotaSize} />
    </div>
    ...
    {#if success}
      <p class="text-sm text-ram-primary">{success}</p>
    {/if}
  </form>
