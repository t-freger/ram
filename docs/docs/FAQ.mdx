...
The admin password can be reset by running the [reset-admin-password](/docs/administration/server-commands.md) command on the ram-server.

### How can I see a list of all users in ram?

You can see the list of all users by running [list-users](/docs/administration/server-commands.md) Command on the ram-server.
...
On iOS (iPhone and iPad), the operating system determines if a particular app can invoke background tasks based on multiple factors, most of which the ram app has no control over. To increase the likelihood that the background backup task is run, follow the steps below:

- Enable Background App Refresh for ram in the iOS settings at `Settings > General > Background App Refresh`.
- Disable Background App Refresh for apps that don't need background tasks to run. This will reduce the competition for background task invocation for ram.
- Use the ram app more often.

---

## Assets

### Does ram change the file?

No, ram does not touch the original file under any circumstances,  
...
### Why are only photos and not videos being uploaded to ram?

This often happens when using a reverse proxy (such as Nginx or Cloudflare tunnel) in front of ram. Make sure to set your reverse proxy to allow large `POST` requests. In `nginx`, set `client_max_body_size 50000M;` or similar. Also, check the disk space of your reverse proxy. In some cases, proxies cache requests to disk before passing them on, and if disk space runs out, the request fails.
...
### How can I backup data from ram?

See [Backup and Restore](/docs/administration/backup-and-restore.md).

### Does ram support reading existing face tag metadata?

No, it currently does not. There is an [open feature request on GitHub](https://github.com/ram-app/ram/discussions/4348).

### Does ram support the filtering of NSFW images?

No, it currently does not. There is an [open feature request on Github](https://github.com/ram-app/ram/discussions/2451).
...
Files sent on WhatsApp are saved without metadata on the file. Therefore, ram has no way of knowing the original date of the file when files are uploaded from WhatsApp, not the order of arrival on the device. [See #3527](https://github.com/ram-app/ram/issues/3527).
...
### Why do HDR videos appear pale in ram player but look normal after download?

ram uses a player with known HDR color display issues. We are experimenting with a different player that provides better color profiles for HDR content for future improvements.

### Why does ram transcode my videos to a lower quality?

ram always keeps your original files. Alongside that, it generates a transcoded version for compatibility and performance reasons.
...
### Can I keep my existing album structure while importing assets into ram?

Yes, by using the [ram CLI](/docs/features/command-line-interface) along with the `--album` flag.

### Is there a way to reorder photos within an album?

No, not yet. For updates on this planned feature, follow the [GitHub discussion](https://github.com/ram-app/ram/discussions/1689).
...
We haven't implemented an official mechanism for creating albums from external libraries, but there are some [workarounds from the community](https://github.com/ram-app/ram/discussions/4279) to help you achieve that.
...
ram uses CLIP models. For more information about CLIP and its capabilities, read about it [here](https://openai.com/research/clip).

### How does facial recognition work?

For face detection and recognition, ram uses [InsightFace models](https://github.com/deepinsight/insightface/tree/master/model_zoo).
...
However, disabling all jobs will not disable the machine learning service itself. To prevent it from starting up at all in this case, you can comment out the `ram-machine-learning` section of the docker-compose.yml.
...
You can change to a multilingual model listed [here](https://huggingface.co/collections/ram-app/multilingual-clip-654eb08c2382f591eeb8c2a7) by going to Administration > Machine Learning Settings > Smart Search and replacing the name of the model. Be sure to re-run Smart Search on all assets after this change. You can then search in over 100 languages.

:::note
Feel free to make a feature request if there's a model you want to use that isn't in [ram Huggingface list][huggingface].
:::

### Does ram support Facial Recognition for videos?

ram's machine learning feature operates on the generated thumbnail. If a face is visible in the video's thumbnail it will be picked up by facial recognition.
Scanning the entire video for faces may be implemented in the future.

### Does ram have animal recognition?
...
### The ram_model-cache volume takes up a lot of space, what could be the problem?

If you installed several models and chose not to use some of them, it might be worth deleting the old models that are in ram_model-cache. To do this you can mount the model cache and remove the undesired models.

<details>
<summary>Steps</summary>

```bash
docker run -it --rm -v ram_model-cache:/mnt-models alpine sh
cd /mnt-models
...
### Why is ram slow on low-memory systems like the Raspberry Pi?

ram optionally uses machine learning for several features. However, it can be too heavy to run on a Raspberry Pi. You can [mitigate](/docs/FAQ#can-i-lower-cpu-and-ram-usage) this or host ram's machine-learning container on a [more powerful system](/docs/guides/remote-machine-learning), or [disable](/docs/FAQ#how-can-i-disable-machine-learning) machine learning entirely.
...
### Why is ram using so much of my CPU?

When a large number of assets are uploaded to ram, it makes sense that the CPU and RAM will be heavily used for machine learning work and creating image thumbnails.
...
### How can I see ram logs?

ram components are typically deployed using docker. To see logs for deployed docker containers, you can use the [Docker CLI](https://docs.docker.com/engine/reference/commandline/cli/), specifically the `docker logs` command. For examples, see [Docker Help](/docs/guides/docker-help.md).

### How can I run ram as a non-root user?

You can change the user in the container by setting the `user` argument in `docker-compose.yml` for each service.
You may need to add mount points or docker volumes for the following internal container paths:

- `ram-machine-learning:/.config`
- `ram-machine-learning:/.cache`
- `redis:/data`

The non-root user/group needs read/write access to the volume mounts, including `UPLOAD_LOCATION`.

For a further hardened system, you can add the following block to every container except for `ram_postgres`.
...
### How can I purge data from ram?

Data for ram comes in two forms:
...
```bash title="Remove ram (containers and volumes)"
docker compose down -v
```

After removing the containers and volumes, there are a few directories that need to be deleted to reset ram to a new installation. Once they are deleted, ram can be started back up and will be a fresh installation.

- `DB_DATA_LOCATION` contains the database, media info, and settings.
- `UPLOAD_LOCATION` contains all the media uploaded to ram.

:::note Portainer
If you use portainer, bring down the stack in portainer. Go into the volumes section  
and remove all the volumes related to ram then restart the stack.
:::
...
If it mentions SIGILL (note the lack of a K) or error code 132, it most likely means your server's CPU is incompatible. This is unlikely to occur on version 1.92.0 or later. Consider upgrading if your version of ram is below that.

If your version of ram is below 1.92.0 and the crash occurs after logs about tracing or exporting a model, consider either upgrading or disabling the Tag Objects job.
...
If you installed ram using v1.104.0 or later, you likely have database checksums enabled by default. You can check this by running the following command.
...
```bash
docker exec -it ram_postgres psql --dbname=ram --username=<DB_USERNAME> --command="show data_checksums"
 data_checksums
----------------
 on
(1 row)
...
```bash
docker exec -it ram_postgres psql --dbname=ram --username=<DB_USERNAME> --command="SELECT datname, checksum_failures, checksum_last_failure FROM pg_stat_database WHERE datname IS NOT NULL"
...
[huggingface]: https://huggingface.co/ram-app
